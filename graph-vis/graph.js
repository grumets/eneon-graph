...
function IsEdgeTypeOfNetwork(link, qedge)
{
	if (!qedge)
		return false;
	for (var i=0; i<qedge.length; i++)
		if (link==qedge[i])
			return true;
	return false;
}
...
function FollowTheLinksOfNetwork(d, nod, filter, links, path)
{
...
function LoopIntoLinksOfNetwork(d, nod, filter, path)
{
	if (filter.direction || filter.direction=="both")
	{
		for (var propName in nod.links)
		{
			if (propName=="source")
				continue;
			if (IsEdgeTypeOfNetwork(propName, filter.excluded_edge))
				continue;
			FollowTheLinksOfNetwork(d, nod, filter, nod.links[propName], path)
		}
		for (var propName in nod.backlinks) 
		{
			if (IsEdgeTypeOfNetwork(propName, filter.excluded_edge))
				continue;
			FollowTheLinksOfNetwork(d, nod, filter, nod.backlinks[propName], path)
		}
		return;
	}
	if (!filter.direction || filter.direction=="forward")
	{
		for (var propName in nod.links)
		{
			if (propName=="source")
		...
		if (filter.bidirectional_edge && filter.bidirectional_edge.length>0)
		{
			for (var propName in nod.backlinks) 
			{
				if (IsEdgeTypeOfNetwork(propName, filter.excluded_edge))
					continue;
				if (IsEdgeTypeOfNetwork(propName, filter.bidirectional_edge))
					FollowTheLinksOfNetwork(d, nod, filter, nod.backlinks[propName], path)
			}
		}
		return
	}	
	if (filter.direction=="back")
	{
		for (var propName in nod.backlinks) 
		{
			if (IsEdgeTypeOfNetwork(propName, filter.excluded_edge))
...
function InitGraph(d, param)
{
	var i_graph=GraphsMM.push(d);
	i_graph--;
	GraphsMM[i_graph].graph_json=param.graph_json;
	GraphsMM[i_graph].div=param.div;

	loadJSON(param.graph_json,
			StartDrawingGraph,
			function(xhr) { alert(xhr); },
			{i_graph: i_graph});
}

function LoadGraph(div, graph_json, conf_vis_json)
{
	loadJSON(conf_vis_json,
			InitGraph,
			function(xhr) { alert(xhr); },
			{div: div, graph_json: graph_json});
}
