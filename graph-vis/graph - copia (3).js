...
function removeDuplicates(list, compare_fn) 
{
	for (var i=0; i+1<list.length; i++)
	{
		var n=0;
		while (i+n+1<list.length && 0==compare_fn(list[i], list[i+n+1]))
			n++;	
		if (n)
			list.splice(i+1, n);
	}
...
function FollowTheLinksOfNetwork(d, nod, filter, links, direction, path)
{
	for (var i_link=0; i_link<links.length; i_link++)
	{
		...
		//Si la cadena és massa llarga la salto
		if (path[path.length-1].length>10)   //Això ho posarem com a paràmetre en algun lloc però ara per arar i poso 10
			continue;
		...
		LoopIntoLinksOfNetwork(d, new_nod, filter, direction, path)
		path[path.length-1].pop();
	}
}

function LoopIntoLinksOfNetwork(d, nod, filter, direction, path)
{
	if (!filter.direction || filter.direction=="forward" || filter.direction=="both")
	{
		for (var propName in nod.links)
		{
			if (propName=="source")
				continue;
			FollowTheLinksOfNetwork(d, nod, filter, nod.links[propName], direction, path)
		}
	}
	if (filter.direction=="back" || filter.direction=="both")
	{
		for (var propName in nod.backlinks) 
			FollowTheLinksOfNetwork(d, nod, filter, nod.backlinks[propName], direction, path)
	}	
...
/*
'selected_nodes' returns a data structure with {selected: true} the nodes that comply with the selected filter and other caracteristics depending on param.
'param.flags' can be:
ini_node:true select only the ini_node
end_node:true select only the end_node (in the past we used filter.type=="extreme" that is now deprecated)
list_end_nodes: true (only if ini_node:true) adds to the selected_node[] a member endNodes[] that contains the array of endnodes.
if no flags are provided it selects all nodes in the path
*/
	...
	for (var i=0; i<network.length; i++)
	{
		...
		if (IsNodeTypeOfNetwork(nod, filter.ini_node))
		{
			if (param && param.ini_node_name && param.ini_node_name!=nod.id)
				continue;
			var path=[[i]];
			LoopIntoLinksOfNetwork(d, nod, filter, filter.direction, path);
			path.pop();
			
			if (path.length==0 && param && param.flags && param.flags.connect===false)
			{
				selected_nodes[i]={selected: true};
			}
			else
			{
				for (var j=0; j<path.length; j++)
				{
					if (param && param.flags)
					{
						...
						{
							if (!selected_nodes[path[j][0]])
								selected_nodes[path[j][0]]={selected: true, endNodes:[]};
							if (param.flags.list_end_nodes)
								selected_nodes[path[j][0]].endNodes.push(path[j][path[j].length-1]);
						}
						if (param.flags.end_node)
							selected_nodes[path[j][path[j].length-1]]={selected: true};
						continue;
					}
					for (var p=0; p<path[j].length; p++)
						selected_nodes[path[j][p]]={selected: true};
				}
			}
		}
	}
	if (param && param.flags && param.flags.ini_node && param.flags.list_end_nodes)
	{
		for (var i=0; i<network.length; i++)
		{
			if (selected_nodes[i] && selected_nodes[i].endNodes && selected_nodes[i].endNodes.length>0)
			{
				//Ordeno i elimino possible repeticions
				selected_nodes[i].endNodes.sort(sortNumber);
				removeDuplicates(selected_nodes[i].endNodes, sortNumber);
			}
		}
	}
}
...
var opt=document.createElement("option");
	...
	ApplyFilterNetwork(graph.d, filter, selected_nodes, {flags: {ini_node: true, list_end_nodes: true}});
	...
	for (var i=0; i<network.length; i++)
	{
			...
			((filter.connect && selected_nodes[i]) || (!filter.connect && !selected_nodes[i])))
		{
			...
			//Aquí escriu la llista de les ocurrencies d'una determinada consulta.
			cdns.push("<li>", nod.title, " <i>(", removeAbbrNamespace(nod.id), ")</i> <b>Type:</b>", nod.type);
			if (filter.connect && selected_nodes[i].endNodes && selected_nodes[i].endNodes.length>0)
			{
				cdns.push(" <b>Connected to: </b>");
				for (var e=0; e<selected_nodes[i].endNodes.length; e++)
				{
					var end_nod=network[selected_nodes[i].endNodes[e]];
					cdns.push(end_nod.title," ", end_nod.type, " <i>(", removeAbbrNamespace(end_nod.id), ")</i>");
					if (e<selected_nodes[i].endNodes.length-1)
						cdns.push(", ");
				}
			}
			cdns.push("</li>");
...
function IniciaGraph(div, graph_json)
{
	var i_graph=GraphsMM.push({
		...
		filter: [{
			...
			//type: "connected",  //Deprecated.
			connect: true,  //false means the ones not connected by the ini_node and the end_node
		...
		{
			//Connecta una EONetwork amd una SDG indicator sense passar per un SDG
			description: "All EO networks connected from SDG indicators",
			connect: true,
			direction: "back", //Can be "forward" (default), "back" or "both" ("both" genera molts camins i no acaba mai)
		...
		{
			//Connecta un EBV amb una EONetwork sense passar per un SDG_indicator
